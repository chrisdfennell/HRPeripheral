...
public class HrGraphView : View
{
    ...
    readonly Paint _zonePill = new() { AntiAlias = true };
    readonly Paint _zoneText = new()
    {
        Color = Color.Black,
        TextSize = 18f,
        FakeBoldText = true,
    ...
    /// <summary>Set the user's age for heart rate zone calculation.</summary>
    public void SetAge(int age)
    {
        _age = HrpPrefs.ClampAge(age);
        Invalidate();
    }
    ...
    protected override void OnDraw(Canvas canvas)
    {
        ...
        if (_lastHr > 0)
        {
            ...
            if (zone > 0)
            {
                ...
                string zoneLabel = $" Z{zone} ";
                _zoneText.GetTextBounds(zoneLabel, 0, zoneLabel.Length, _textBounds);
                float zx = left + 4;
                float zy = top + _textBounds.Height() + 4;
                _zonePill.Color = Color.Rgb(r, g, b);
                canvas.DrawRoundRect(zx - 2, zy - _textBounds.Height() - 2,
                                     zx + _textBounds.Width() + 2, zy + 4,
                                     6, 6, _zonePill);
                canvas.DrawText(zoneLabel, zx, zy, _zoneText);
            }
        }
