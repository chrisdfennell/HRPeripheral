...
[Activity(
    ...
    ScreenOrientation = ScreenOrientation.Portrait   // <-- This should be the only orientation line
)]
public class MainActivity : Activity
{
    ...
    private Java.Lang.IRunnable? _hapticTick;      // single self-rescheduling haptic
    ...
    {
        // Single self-rescheduling runnable (instead of N anonymous ones that can't be cancelled)
        _hapticTick = new Java.Lang.Runnable(() =>
        {
            if (!_holding) return;
            long elapsed = Java.Lang.JavaSystem.CurrentTimeMillis() - _holdStart;
    ...
    private class HrUpdateReceiver : BroadcastReceiver
    {
        ...
        public override void OnReceive(Context? context, Intent? intent)
        {
            ...
            // Show BLE errors as Toast
            string? error = intent.GetStringExtra("error");
            if (!string.IsNullOrEmpty(error))
            {
                Toast.MakeText(_host, $"BLE: {error}", ToastLength.Long)?.Show();
            }
