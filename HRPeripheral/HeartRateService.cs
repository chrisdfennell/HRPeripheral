...
public class HeartRateService : Service, ISensorEventListener
{
    ...
    // Auto-pause: loaded from preferences at startup (default: off)
    ...
    private const int BATTERY_UPDATE_INTERVAL_MS = 60_000; // 60 seconds
    ...
    public override void OnCreate()
    {
        ...
        // CalorieEstimator will be initialized after prefs are loaded (below)
        ...
        // Subscribe to BLE events for user-facing feedback
        _ble.OnError += (msg) =>
        {
            LogE($"BLE error event: {msg}");
            UpdateNotificationText($"Error: {msg}");
        ...
        {
            LogI($"BLE status event: {msg}");
            if (msg.Contains("subscribed", StringComparison.OrdinalIgnoreCase))
            {
                UpdateNotificationText(_currentHr > 0
                    ? $"HR: {_currentHr} bpm | {_kcal:F0} kcal | Connected"
                    : "Client connected - waiting for HR...");
            }
            else if (msg.Contains("disconnected", StringComparison.OrdinalIgnoreCase))
            {
                UpdateNotificationText(_currentHr > 0
                    ? $"HR: {_currentHr} bpm | {_kcal:F0} kcal"
    ...
    private void BroadcastUpdate()
    {
        ...
        if (_currentHr > 0)
            UpdateNotificationText($"HR: {_currentHr} bpm | {_kcal:F0} kcal");
        else
            UpdateNotificationText("Waiting for heart rate...");
        ...
        intent.PutExtra("kcal", _kcal);
        intent.PutExtra("paused", _paused);
        intent.PutExtra("zone", HeartRateZone.GetZone(_currentHr, _calAge));
        intent.SetPackage(PackageName);
        try
        {
            SendBroadcast(intent);
